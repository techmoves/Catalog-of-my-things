...
describe Game do
  ...
  describe '#initialize' do
    ...
    it 'the new Game\'s publish date should be 1990-03-23' do
      expected_time = Time.new(1999, 3, 23)

      expect(game.publish_date).to eq(expected_time)
    end

    it 'the new Game\'s last_played_at should be 2005-01-01' do
      expected_time = Time.new(2005, 1, 1)
  ...
  describe '#can_be_archived?' do
    ...
    it 'shoud return nothing if published date is less that 10 years ago' do
      game = Game.new(true, Time.new(2005, 1, 1), Time.new(2021, 3, 23), true)

      expect(game.can_be_archived?).to eq(nil)
    end

    it 'shoud return nothing if last_played_at variable was less than 2 years ago' do
      game = Game.new(true, Time.new(2023, 1, 1), Time.new(1999, 3, 23), true)
